...
bool I2C::read(uint8_t address, uint8_t reg, uint8_t* buffer, uint8_t length){
    ...
    while(_state != I2C_STATE_IDLE); //maybe return for non-blocking?

    
    _packet.write_address = (address << 1);
    _packet.read_address = (address << 1) | 1;
    _packet.reg = reg;
    _packet.buffer_ptr = buffer;
    _packet.length = length;
    _packet.index = 0;
    _packet.error_counter = 0;

    _state = I2C_STATE_WRITE_REG;
    _i2c->CR1 |= I2C_CR1_START;

    return true;
}

void I2C::readIRQhandler(){
    if(_i2c->SR1 & I2C_SR1_SB) {
        if (_state == I2C_STATE_WRITE_REG){
            _i2c->DR = _packet.write_address;
        } 
        else if (_state == I2C_STATE_READ){
            _i2c->DR = _packet.read_address;
        }
    }
    ...
    if(_i2c->SR1 & I2C_SR1_ADDR){
        ...
        if (_state == I2C_STATE_READ) {
            if (_packet.length == 1) {
                // Special case: 1-byte read
                _i2c->CR1 &= ~I2C_CR1_ACK; // disable ACK
                _i2c->CR1 |= I2C_CR1_STOP; // generate STOP right after this byte
            } else {
                _i2c->CR1 |= I2C_CR1_ACK; // multi-byte: enable ACK
            }
        }
    }

    if((_i2c->SR1 & I2C_SR1_TXE) && _state == I2C_STATE_WRITE_REG) {
        _i2c->DR = _packet.reg;                 // send register
        _i2c->CR1 |= I2C_CR1_START;        // repeated start for read
        _state = I2C_STATE_READ;
    }

    // RXNE: data received
    if ((_i2c->SR1 & I2C_SR1_RXNE) && _state == I2C_STATE_READ) {
        _packet.buffer_ptr[_packet.index++] = _i2c->DR;

        if (_packet.index == _packet.length - 1) {       // next is last byte
            _i2c->CR1 &= ~I2C_CR1_ACK;     // NACK last byte
            _i2c->CR1 |= I2C_CR1_STOP;     // Generate STOP
        }

        if (_packet.index == _packet.length) {
            _state = I2C_STATE_IDLE;     // transaction complete
        }
    }
}
